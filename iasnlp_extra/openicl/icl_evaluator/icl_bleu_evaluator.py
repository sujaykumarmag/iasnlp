...
from src.openicl.utils.metrics import get_ngrams
...
#         # Download and load the SentencePiece model
#         url = "https://dl.fbaipublicfiles.com/fairseq/models/flores/sacrebleu_tokenizer_spm.model"
#         model_path = 'sacrebleu_tokenizer_spm.model'
        
#         # Set up the SSL context to use certifi certificates
#         context = ssl.create_default_context(cafile=certifi.where())
        
#         # Download the model with SSL context
...
class IclBleuEvaluator(IclBaseEvaluator):
    ...
    def calculate_bleu(self, references: Dict[str, List[List[str]]],
        ...
        # Tokenize the reference and prediction corpora using SentencePiece
        _reference_corpus = [" ".join(self.tokenizer.encode_as_pieces(" ".join(tok_list))) for tok_list in reference_corpus]
        _prediction_corpus = [" ".join(self.tokenizer.encode_as_pieces(" ".join(pred))) for pred in prediction_corpus]
        
        
        with open(self.output_dir+'/{}_{}_prediction.txt'.format(src_lang, tgt_lang), 'w') as f:
            f.writelines(_prediction_corpus)

        with open(self.output_dir+'/{}_{}_ref.txt'.format(src_lang, tgt_lang), 'w') as f:
            f.writelines(_reference_corpus)
        ...
        score = sacrebleu.corpus_bleu(_prediction_corpus, [_reference_corpus], tokenize="none")
        print(score)
